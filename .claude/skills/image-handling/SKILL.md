...
`AvatarUploadModal`/`BackgroundUploadModal` aren't exclusive to `ManageUserProfile` — `LoginForm`'s Create Profile step (`src/features/auth/screens/LoginForm.tsx`, the invite flow's final screen, Figma 547:2289) mounts the same two components pre-onboarding, right after Google OAuth. This works because a `profiles` row already exists for the user by then (the `handle_new_user` signup trigger creates it), so `updateAvatarAction`/`updateBackgroundAction`'s `.update(...).eq('id', userId)` has a row to hit — don't assume these modals are exclusive to already-onboarded users when reusing them in a new pre-onboarding surface.
