...
- `icons/pin-heart.svg` / `pin-heart-tertiary.svg` / `pin-door.svg` — Pin Squad's custom pixel-art badge/action icons (`SwipePreviewCard`'s pinned-card badge, `RoomPinSheet`'s Pin/Leave Squad buttons — see CLAUDE.md's Pin Squad section). Downloaded straight from Figma rather than pixelarticons glyphs, since none of that library's icons match these shapes and `pin-heart`'s fill is the two-stop `--gradient-nexus` gradient baked into the file (not `currentColor`-driven) — `pin-heart-tertiary` is a flat `--color-tertiary`-filled copy of the same shape for `SheetActionButton`'s `disabled` state, since an externally-referenced `<img src="…svg">` can't inherit the button's CSS color the way an inline SVG would.
...
- `supabaseImageLoader({ src, width, quality })` — standard `next/image` loader. Rewrites `/storage/v1/object/public/` → `/storage/v1/render/image/public/`, sets `width` + `resize=contain` + `quality` (default 75). No-ops for non-Supabase URLs.
  - **`resize=contain` is required, not optional.** Verified directly against the render endpoint: given only `width` (no `height`/`resize`), Supabase's transform API does **not** scale proportionally — it resizes the width to the requested value but leaves height at the source's original pixel value. A 1080×608 (16:9) photo requested at `?width=360` came back **360×608**, a portrait-squished image at a completely different aspect ratio. Every `object-fit` on top of that was working against a malformed source — this is what actually caused `UserCard`/`ProfileHeroBackground`'s background images to look zoomed in, not a container aspect-ratio mismatch (that theory was tried first and didn't hold up). `?width=360&resize=contain` correctly returns 360×203.
  - `avatarImageLoader` is unaffected — passes `width` **and** `height` explicitly, and empirically the render endpoint's default (no `resize` param) is byte-identical to explicit `resize=cover` in that two-dimension case, so it was always hard-cropping correctly. The bug is specific to the width-only path.
- `avatarImageLoader({ src, width, quality })` — forces a square crop: for Supabase storage sets both `width` and `height` to the same value; for `googleusercontent.com` URLs rewrites the `=sNNN` suffix to `=s{width}-c`; other URLs pass through unchanged. Used by `UserAvatar` and `LinkPill`'s vinyl-type 12×12 disc (both squares, `fill` + `object-cover`). A custom `loader` prop bypasses `next/image`'s `images.remotePatterns` domain check entirely (that validation lives inside `defaultLoader` only), so `LinkPill` can safely route arbitrary external OG-thumbnail hosts through it — the pass-through branch just returns the URL unchanged, identical to the plain `<img>` it replaced.
...
- `<ProfileHeroBackground>` — full-bleed hero image (resizes to 480w via `supabaseImageLoader` before rendering as a plain `<img>`), takes a generic `url: string | null` prop despite the "Profile" name — originally built for `profiles.background_url` (`ProfileClient`/`AccountPageMember`/`MemberProfileClient`), and reused as-is (no crew-specific variant needed) by `JoinGroupStep`'s invite-success screen for `crews.background_image_url`, since the underlying markup (absolute-positioned cover `<img>`, `supabaseImageLoader`, `/img/default_image.png` fallback) doesn't care whose background it is. `UserCard`'s member-card background and `ManageUserProfile`'s hero preview use the same plain-`<img>` inline pattern (not this shared component, since their container dimensions/gradient differ) — all three profile-scoped surfaces render the same `background_url`, cropped to 16:9 at upload time (`BackgroundUploadModal`'s fixed pan/zoom frame).
  - **`object-fit`**: all three background_url surfaces (`UserCard` 180×108 ~1.67:1, `ManageUserProfile`'s hero 240px tall ~1.6:1, `ProfileHeroBackground`'s hero `280px + safe-area-inset-top` ~1.15–1.26:1 on a notched phone) use `object-fit: cover` — scale to fill height, width scales proportionally with it, overflow clipped. `ProfileHeroBackground`'s container is the tallest/narrowest of the three relative to the source's 16:9, so `cover` crops more of the photo's width there than on the other two — expected given the different container shapes, not a bug. It briefly used `object-fit: fill` (stretch, no crop) as a workaround while `supabaseImageLoader` was silently returning a non-proportional (wrong-aspect-ratio) source — now that the loader itself is fixed (`resize=contain`, above), `cover` is correct again everywhere. Don't reintroduce `fill` without first re-checking the actual bytes `supabaseImageLoader` returns (`curl` the render endpoint URL + `file`) — a `fill` "fix" is a strong signal something upstream is still returning a malformed image.
...
| Crew-creation profile photo | `CreateSquadPage` (`/home/create`, `handleProfilePhotoCropConfirm`) | 128px + 256px |
| Crew-creation background | `CreateSquadPage` (`/home/create`, `handleBackgroundCropConfirm`) | 1 (1080×608) |
...
`AvatarUploadModal`/`BackgroundUploadModal` aren't exclusive to `ManageUserProfile` — `CreateProfileStep` (`src/features/auth/screens/CreateProfileStep.tsx`, LoginForm's `create-profile` step, header text "Setup Profile", Figma 774:20648) mounts the same two components pre-onboarding, right after Google OAuth. This works because a `profiles` row already exists for the user by then (the `handle_new_user` signup trigger creates it), so `updateAvatarAction`/`updateBackgroundAction`'s `.update(...).eq('id', userId)` has a row to hit — don't assume these modals are exclusive to already-onboarded users when reusing them in a new pre-onboarding surface. This screen's own upload *triggers* (a dashed circle/rectangle + centered pixelarticons `Plus` icon, sized to fill its slot — not the shared "Upload" button `ManageUserProfile`/`ManageSquadProfile` render) are the same convention `CreateSquadPage` (`/home/create`, Figma 784:24486) reuses for its own Group Photo/Background Image triggers (crop pipeline unrelated — that screen's uploads go straight to Supabase Storage + its own server actions, not `AvatarUploadModal`/`BackgroundUploadModal`, since a crew doesn't exist to attach them to until `handleCreate` runs). If you're building a new pre-content-exists upload surface (nothing to attach the image to yet), reach for this dashed+`Plus` trigger shape rather than the filled "Upload" pill button, which is for editing an *existing* record.
...
- **`PhotoCropModal` self-portals to `document.body`** (`createPortal`, `mounted`-gated) — it doesn't render in place. Every real call site nests it inside something with an active Framer Motion `transform` (a `SlidePage` page-slide, or a `BottomSheet`'s own motion wrapper), which per the CSS spec becomes the containing block for its `position: fixed` sheet/backdrop, silently mispositioning/clipping it against that ancestor's box instead of the true viewport (see CLAUDE.md's Gotchas — the same bug class as `VinylActionSheet`). This is what caused `CreateSquadPage`'s crop sheet to visually overlap the page's own footer button on iOS. Don't remove the portal to "simplify" the component, and don't re-solve this per-caller with a `key`/`z-index` tweak — the fix belongs in this shared component exactly because every caller nests it inside something animated.
