...
- `<ProfileHeroBackground>` — full-bleed `profiles.background_url` hero (resizes to 480w via `supabaseImageLoader` before rendering as a plain `<img>`). `UserCard`'s member-card background and `ManageUserProfile`'s hero preview use the same plain-`<img>` inline pattern (not this shared component, since their container dimensions/gradient differ) — all three render the same `background_url`, cropped to 16:9 at upload time (`BackgroundUploadModal`'s fixed pan/zoom frame).
  - **`object-fit` differs deliberately by surface, not a bug to unify.** `UserCard` (180×108, ~1.67:1) and `ManageUserProfile`'s hero (240px tall, ~1.6:1) are both close enough to the source's 16:9 that `object-fit: cover` barely crops — use `cover` there. `ProfileHeroBackground` (used by `ProfileClient`/`MemberProfileClient`/`AccountPageMember`, `280px + safe-area-inset-top` tall, ~1.15–1.26:1 on a notched phone) is much taller/narrower than 16:9 — `cover` there was cropping away most of the photo's width, making the same image look far more zoomed in than on Manage Profile. Fixed by using `object-fit: fill` instead: keeps the full width in frame (same horizontal framing as the other two surfaces) and stretches vertically to cover the extra height rather than cropping the sides off. A mild vertical stretch beats an inconsistent crop here. If a new hero surface's aspect ratio is close to 16:9, use `cover`; if it's meaningfully taller/narrower than 16:9, use `fill` to stay visually consistent with the other surfaces instead of introducing a new zoom level.
