...
`NotifSheet` also renders a **Re-enable** button below the three toggles (gated on `isSupported()` from `src/shared/utils/notifications.ts`) — this is unrelated to `NotifPrefs`/`crew_notification_preferences`. It's a device-level push-subscription action: tapping it runs `requestPermission()` then `subscribeToPush()` (the same pair `NotificationPrompt`'s Enable flow calls), which re-prompts OS permission and re-upserts this device's own `push_subscriptions` row. It doesn't touch the per-crew mute toggles, and it has no effect on any other device or user. It's self-contained inside `NotifSheet` (its own `resubscribing` state) — `ChatInput` doesn't own or pass anything for it.
```ts
...
- `src/features/chat/components/sheets/NotifSheet.tsx` — `NotifPrefs` type, `NotifToggleRow`, `NotifSheet` (presentational for the toggles; `ChatInput.tsx` owns that state and the Supabase calls). Also self-contains its own **Re-enable** button (device push resubscribe via `requestPermission()`/`subscribeToPush()`) — see "Settings UI" above.
...
- `src/features/chat/hooks/usePresenceChannel.ts` — the only call site for `notifyActiveCrew`, piggybacked on the existing per-crew presence effect (this hook is called from `ChatInput.tsx`, but the calls themselves live here — see CLAUDE.md's ChatInput section for the full hook breakdown)
...
- **A user no longer has to wait for a dev to fix a stale/missing subscription.** `NotifSheet`'s Re-enable button lets the user themselves re-run `requestPermission()` + `subscribeToPush()` on their own device from inside the app, on demand — the same self-heal the diagnostics page's per-user refresh button forces server-side (delete rows, wait for next `PushRefresh` mount), just user-triggered and immediate rather than dev-triggered and next-open. It's the client-side counterpart to that admin action, not a replacement for it — the admin refresh still exists for when a dev needs to force it on someone else's device.
...
- Only `usePresenceChannel` (mounted by `ChatInput.tsx`) calls `notifyActiveCrew`. If a future screen needs to represent "user is looking at crew X" without mounting `ChatInput`/`usePresenceChannel` (e.g. a preview/quick-glance surface), it must call `notifyActiveCrew` itself at mount/visibility/unmount — this is not automatically derived from routing or any global store.
...
- `send-notification/index.ts` changes require a manual `supabase functions deploy send-notification --project-ref tlveyeisjbythssmocth` — `sw-push.js`/`usePresenceChannel.ts`/`notifications.ts` changes ship on the normal `git push` → Vercel deploy.
