...
- **JoinGroupStep's invite-success full-page background (Figma 784:5792).** A full-viewport background (not a card thumbnail) needs a steeper, bottom-heavy fade so the header/card/form stay legible over an arbitrary crew photo — `--gradient-image-overlay`'s 20%→80% curve is tuned for a small cover thumbnail, not a whole screen. `JoinGroupStep.tsx`'s local `PAGE_BACKGROUND_GRADIENT` constant (still built from `color-mix(in srgb, var(--color-background) …, transparent)`, matching this token family's own convention — just different stops: 0/20/50/70/80% opacity at 0/51/69/80/100%) is reused for BOTH the page background and, via `SwipePreviewCard`'s `overlayGradient` override prop, that screen's own card — Figma applies the identical gradient to both, and forcing either onto `--gradient-image-overlay` would visibly mismatch the design.
