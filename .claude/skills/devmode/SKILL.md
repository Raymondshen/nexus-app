...
`nexus_dev_mode`, `nexus_push_diag`, `nexus_infinite_coins`, `nexus_afk_exp`, `nexus_friendship_xp`, `nexus_poll_feature`, `nexus_events_enabled`, `nexus_combat_system` are **per-device UI toggles for cosmetic/debug display**, not access control. `nexus_infinite_coins`, `nexus_friendship_xp`, `nexus_events_enabled`, and `nexus_poll_feature` have an in-app toggle row in Developer Settings (itself is_dev-gated, Pattern A) — flipping one only ever changes what renders in *your own* client, and any action it unlocks (e.g. creating an event) is separately re-checked server-side via Pattern C. `nexus_dev_mode`, `nexus_afk_exp`, `nexus_combat_system`, and (as of the Figma 708:18773 redesign) `nexus_push_diag` have no in-app toggle at all — devtools-only (or, for `nexus_dev_mode`, settable via a `?dev=1` URL param handled in `(app)/layout.tsx`). `nexus_push_diag`'s toggle row used to live in Developer Settings; it was removed when that page was redesigned to match Figma, since the real per-user notification debugging surface (`/profile/developer/manage-notifications`, Pattern A) replaced its purpose — the flag itself still gates `PushDebugFAB` exactly as before.

**Known, accepted characteristic — not a bug to "fix" reflexively:** a couple of purely-visual dev toggles (`PushDebugFAB`'s `nexus_push_diag` gate, `ChatFloatingNav`'s `nexus_dev_mode`/`nexus_events_enabled` gate) check *only* the localStorage flag, with no `isDev` prop in that component at all. This is acceptable **only** because: (a) the flag has no discoverable in-app path for a non-dev user to flip on their own device, and (b) nothing sensitive is exposed if they did — `PushDebugFAB` only reveals your *own* push-subscription diagnostics, `ChatFloatingNav`'s gate only reveals a client-only events preview UI. If a future feature behind one of these flags starts exposing another user's data, another user's actions, or anything mutating, it must be upgraded to Pattern B/C — don't assume "it's behind a devtools flag" is sufficient once the stakes change.

**`/profile/developer/manage-notifications`'s "Remove User Completely" action is the sharpest example of Pattern C in this codebase** — it's a full, immediate, irreversible cross-user account deletion (calls the `process-deletions` edge function's cascade directly, no grace period). Its server action (`adminDeleteUserAction`, `manage-notifications/actions.ts`) opens with the same `requireDev()` check as everything else here, plus an extra self-delete guard (`targetUserId === caller's own id` → error) that has nothing to do with dev-gating — a reminder that Pattern C is "verify the caller may do this at all," not a substitute for feature-specific safety checks a truly destructive action still needs on top.
...
- `src/features/profile/screens/DeveloperUserSettings.tsx` — the toggle rows for `nexus_infinite_coins`/`nexus_poll_feature`/`nexus_events_enabled`/`nexus_friendship_xp`, plus the elevated nav rows into Announcements and Manage Notifications
- `src/app/(app)/profile/developer/manage-notifications/actions.ts` — its own `requireDev()` copy; `adminDeleteUserAction` is the sharpest Pattern C example (see above)
- `src/shared/components/pwa/PushDebugFAB.tsx`, `ChatFloatingNav` in `src/shared/components/ui/PageFloatButton.tsx` — the accepted localStorage-only exceptions (self-scoped, non-mutating)
