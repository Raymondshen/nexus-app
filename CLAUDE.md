...
## Auth
- Google OAuth: `signInWithOAuth` → `/auth/callback` → `/home` (or `/login?newAccount=1` for a brand-new account — see Login)
...
**Create Profile** (`create-profile` step, Figma 547:2289) — modeled on `ManageUserProfile`'s layout: live hero (real `coins`/`gem_balance` from `getSignupSessionAction`'s `SessionProfileSnapshot`; "Lifetime msg." hardcoded 0 — always true pre-crew-join) → status ticker → read-only Account email → Profile Photo/Background Image upload (`AvatarUploadModal`/`BackgroundUploadModal`, same components `ManageUserProfile` uses — see the `image-handling` skill) → Display Name (→ `username`) → First Name/Last Name (kept even though the Figma omits them — nothing else in the app surfaces them, but the field stays available) → Current Mood (→ `status`) → Social Links (5 fields, see `profiles` table). "Create Profile" submits through `completeSignupAction`'s `extra: CompleteSignupExtra` param (status + the 5 social links, normalized server-side via `normalizeSocialUrl()`) and routes to `/home`.
...
│   ├── home/                   HomeClient, CreateSquadPage, homePreviewCache.ts
