...
- **Fixed header** (`flex-shrink-0 flex flex-col gap-4 pt-[var(--space-7)] px-4`, 24px top padding via `--space-7`):
  - *Content block* (`flex flex-col gap-14` = 56px gap):
    - *Title row* (`flex items-start justify-between`): left group — 32×32 group profile image (solid `bg-purple` default; creator taps to upload via `CrewImageUploadModal`) + crew name in Silkscreen `var(--text-md)` (16px) `text-purple` (editable by creator) + member count in Silkscreen 8px `text-tertiary` below. Right group — `flex items-center gap-4`: `Bell` 24×24 `text-primary` (fires `onNotifPress` prop), `UserPlus` 24×24 `text-primary` (fires `onSharePress` prop, only when `inviteCode` present), `ChevronRight` rotated `90deg` 24×24 `text-tertiary` — taps to collapse. `onNotifPress` and `onSharePress` are optional props on `ChatInput`.
