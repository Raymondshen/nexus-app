...
### SquadDetailsSheet (`src/features/chat/components/sheets/SquadDetailsSheet.tsx`, Figma 596:8291)
**Overlay pattern, not the docked "Panel" pattern anymore** — as of the Figma 596:8291 redesign, this shares the exact same overlay + transition as `ChatRoomBrowseSheet` (see that component's own doc comment) rather than being a bottom-sheet sliding up from `y: '100%'`: a single `motion.div`, `fixed left-0 right-0 top-0` down to `bottom: chatInputHeight` (leaving the composer visible below, same as the browse sheet), `bg-black/85` backdrop, opacity-only enter/exit (no y-slide), and the same `useSheetDrag` pull-to-close with `dragElastic={{ top: 0, bottom: 0 }}` zeroed out. Tapping the backdrop (anywhere outside the header row, hero/invite card, and the member row) calls `onClose`; those three content blocks stop click propagation. No more separate backdrop-`motion.div` + docked-panel-`motion.div` pair — dismissal is backdrop-tap/drag/explicit-close only, matching the browse sheet.

**Header row** (Figma 599:3842, `flex-shrink-0`, never scrolls) — crew name (plain uppercase title text) + the action icon row, right-aligned: `MagicEdit` (creator only) + `Bell`/`BellOff` (opens `NotifSheet`, owned by `ChatInput`) + `Library` (navigates to `/chat/${crewId}/definitions`) + a trailing `ChevronDown` that's an explicit close button calling `onClose`. This action row used to live inside the hero's top-right corner (pre-Figma-599:3842 revision); it moved out to this fixed header so the actions stay reachable regardless of how far the body below is scrolled.

**Scrollable body** — everything below the header row (group card details, member row, Leave Squad) is wrapped in its own `flex-1 min-h-0 overflow-y-auto nexus-scroll` region, same idiom as the Squad Updates page's card-list region (`AnnouncementsSheet.tsx`). This nesting is required, not cosmetic: `useSheetDrag`'s `canDragFrom` ancestor-walk stops at (excludes) the sheet root itself, so making the *root* the scrollable element would mean the walk can never detect it, and every vertical touch on the body would be hijacked into the pull-to-close drag instead of scrolling. Nesting the scrollable region one level below the root — the same shape `BottomSheet` callers and `AnnouncementsSheet` already use — is what lets the walk find it and gate the drag on `scrollTop <= 0`.

Body layout (flex col, `gap: var(--space-5)`, same outer padding as `ChatRoomBrowseSheet`):
1. **Group card details** (Figma 596:8296) — hero + invite section combined into one rounded (`var(--x3)`, all four corners) `--color-surface-sheet` card:
   - **Hero** (`aspect-ratio: 393/240`) — background + gradient overlay (`var(--gradient-image-overlay)`, unchanged); a single bottom-anchored row: 40px `GroupAvatar` + crew name (uppercase, `--color-secondary`) + `{totalMessages} total Squad msg.` + XP progress bar. No action buttons here anymore (see Header row above). The old `Lv.{n} · {count} members` line and the numeric XP-fraction text are gone — Figma's heading row doesn't show them (crew level is still visible on the collapsed `ChatSquadDetailBar` below).
   - **Invite section** — `<InviteCodeCard>` (Figma 438:8098; same shared component as `MessageList`'s empty state — don't re-inline this markup a third time). Its border is purple (`border-purple`), matching Figma's `border-[#a855f7]` spec on both call sites — a pre-existing drift to a plain gray `border-border` was fixed here.
2. **Member row** (Figma 596:8481, no longer under a "Members" label — Figma doesn't show one) — a horizontally-scrollable row (`overflow-x-auto no-scrollbar`) of member `UserCard`s (Figma 356:3503, 180px wide, every card the same total height by construction — see below). Each card: the member's own `profiles.background_url` as the header image (fallback `/img/default_image.png`, same as `AccountPageMember`/`MemberProfileClient` — NOT the crew's `background_image_url`), rendered as a plain `<img>` with `inset: 0`, `width/height: 100%`, `objectFit: cover` — same full-bleed fill technique as `ProfileHeroBackground` (user profile) and `ManageUserProfile`'s hero preview, all three now on `cover` since the underlying `supabaseImageLoader` non-proportional-resize bug was fixed at the loader level — see the `image-handling` skill + 32px `UserAvatar` (online dot if online) → username → class row (`Crown` 12×12 amber if creator + 12×12 `PixelSprite` (scale `0.5625`) + `{class} · {msgCount} msg.`) → a **reserved vinyl pill slot**: `<LinkPill type="vinyl">` (`src/shared/components/ui/LinkPill.tsx`, shared with `MessageBubble`'s header row) if the member has one, else a blank 20px-tall placeholder (`VINYL_PILL_HEIGHT`, matching `LinkPill`'s own natural height) → a **reserved status ticker slot**: `<TickerBanner>` (see below) if the member has a `status` set, else `BlankTickerSlot` — a placeholder that mirrors `TickerBanner`'s own outer wrapper markup so its rendered height matches exactly. Figma 432:7827 (the real mockup row) shows this precisely: the full card (432:7819, vinyl + ticker) and the `self-stretch` empty card (432:8008) both resolve to 231px, while the one card missing `self-stretch` (432:8021) collapses to 172px — the "collapse" look was the bug, not the design; every card must reserve both slots' space regardless of content, not rely on the row's `align-items: stretch` to paper over the difference. Tapping a card calls `onTapMember` → `/chat/${crewId}/member/${memberId}`.
3. **Leave Squad button** — not part of Figma's 596:8291 crop, but the app still needs a way out of a squad. Kept as a plain flex child below the member row (no docked `SheetFooter` — this overlay has no sheet chrome to dock a footer against, same as `ChatRoomBrowseSheet`'s sections).
...
Invite is surfaced only via the inline `<InviteCodeCard>` in the group card details block (Figma removed the header's `UserPlus` icon in favor of this — don't re-add a separate invite sheet/icon).

**The composer stays visible underneath while this overlay is open** — `ChatInput`'s own squad-bar+input box (the `chatInputBoxRef` box the overlay's `bottom: chatInputHeight` stops above) is never faded/hidden by `isExpanded` (the state that drives this sheet), matching how it already stays visible underneath `ChatRoomBrowseSheet`. It used to fade to `opacity: 0` while expanded — fixed since both overlays occupy the same footprint (`top: 0` down to `bottom: chatInputHeight`) and neither should black out the input row below it.
...
- **`ChatRoomBrowseSheet` and `SquadDetailsSheet`'s root `motion.div` transitions must be changed together, not one at a time.** Both doc comments assert "same overlay + transition as" the other — an easing tweak applied to only one (fixing `ChatRoomBrowseSheet`'s open fade to use `ease: 'easeInOut'` without also updating `SquadDetailsSheet`'s) silently breaks that documented parity. Check both `animate`/`exit` blocks whenever adjusting either one.
- **`ChatRoomBrowseSheet`'s Group Details header icon order has diverged from `SquadDetailsSheet`'s own header** — `ChatRoomBrowseSheet` is `MagicEdit, Library, Bell, Close`, with MagicEdit/Library fading in/out based on `viewingGroupDetails` and Bell/Close always visible; `SquadDetailsSheet` is `MagicEdit, Bell, Library, close`, all four always visible (no fade — that component has no `viewingGroupDetails`-style two-page scroll). Don't assume the two headers are interchangeable when editing one.
