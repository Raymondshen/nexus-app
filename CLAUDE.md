...
│   │   │                       (Squads row + Notifications + Group Details, all in one —
│   │   │                       see its own section)
...
### ChatRoomBrowseSheet a.k.a. "Updates" (`src/features/chat/components/input/ChatRoomBrowseSheet.tsx`, Figma 589:3619 "body")
Covers all three at once — Notifications, Squads quick-switch, and the current room's own Group Details — there is no separate sheet or page for any of these; a standalone `ChatSquadsPage`/`/chat/[crewId]/squads` route existed briefly and was folded back in here. Opened two ways, both toggling the same sheet: a vertical swipe-up anywhere on `chatInputContainer` (`ChatInput`'s `handleTopPanEnd`), or a plain tap anywhere on `ChatSquadDetailBar` (`onTap`, wired to `setShowRoomBrowser((prev) => !prev)` in `ChatInput`) — a second tap while it's open closes it.
...
**Header row** (`flex-shrink-0`, never scrolls, Figma 599:7818 — bold non-uppercase DM Sans, no back chevron): when `squadDetail` is present (non-DM) — the current room's crew name with a decorative leading `ChevronRight`, and `right` = `MagicEdit` (creator only) → `Library` → `Bell`/`BellOff` (opens `NotifSheet`, owned by `ChatInput`) → `Close`. `MagicEdit`/`Library` fade out (`opacity: 0`, `pointerEvents: none`) while `viewingGroupDetails` is false — i.e. still on the Notifications+Squads page below — and fade back in once the user scrolls/snaps onto Group Details, since those actions only make sense once squad context is actually on screen; `Bell`/`Close` stay reachable on both pages. `viewingGroupDetails` is a real `IntersectionObserver` (`root` = the scroll container, `target` = the Group Details wrapper, `threshold: 0.5`), not a scrollTop heuristic. Falls back to a plain "Updates" title + Close-only `right` when `squadDetail` is null (the DM screen — no squad to edit/mute/browse definitions for, so nothing to fade either; DM still gets the Notifications+Squads content below, just no Group Details section).

**Body**: `overflow-y-auto` with `scroll-snap-type: y mandatory` — two snap "pages": Notifications+Squads (height 100% of the scroll container), then Group Details.
1. **Notifications** (Figma 589:4570) — a single card surfacing whichever room has unread messages and received one most recently (`notifRoom`), shown above Squads (explicit reorder from Figma's own top-to-bottom layout). Never hidden — with no unread room it shows a plain "you're all caught up" ghost state (`NoNotificationsCard`) instead. `flex: 1 0 0` so it fills whatever space Squads doesn't use.
2. **Squads row** (Figma 589:3631) — every room in `chatRoomOrder`, native horizontally-scrollable, reusing `SwipePreviewCard`, plus a "Create Squad" card as a genuine first item (`items[0]`, not a bolted-on leading slot) that routes straight to `/home/create`. The pinned room (`profiles.pinned_crew_id`) always sorts to the front, right after Create Squad. Tap a card to navigate (`onSelectRoom` → `ChatInput`'s `commitRoomSwitch`, which seeds `chatRoomPeekStore`'s swipe-nav peek/ghost transition + `skipNextSlideEnter(true)` + `nexus_chat_from` before `router.push`); long-press (500ms, `PIN_LONG_PRESS_MS`) any card opens the Pin/Leave Squad sheet (`RoomPinSheet`) — Leave Squad works for ANY room in the list via `onLeaveRoom`, not just the current one (`ChatInput`'s `requestLeaveSquad`, generalized beyond the current-room-only gate). The header's `ScrollEqualizerBars` track native scroll position live (color: primary if current room, red if unread, else muted; height: tall only for the focused bar) via a sliding window (`EQUALIZER_WINDOW`) centered on `focusedIndex`.
3. **Group Details** (Figma 599:3931/601:4007) — the card + member row + Leave Squad button for whichever room this sheet was opened from (`squadDetail`, threaded down from `ChatInput` rather than derived from `RoomMeta`, which doesn't carry invite codes/per-member data/a real XP fraction). `null` on the DM screen.
   - **Group card details** (`<SquadDetailCard>`) — hero + invite section combined into one rounded (`var(--x3)`) `--color-surface-sheet` card. Hero (`aspect-ratio: 393/240`): background + `var(--gradient-image-overlay)` overlay, bottom-anchored 40px `GroupAvatar` + crew name + `{totalMessages} total Squad msg.` + XP bar. Invite section: `<InviteCodeCard>` (Figma 438:8098, shared with `MessageList`'s empty state — don't re-inline).
   ...
   - A zero-height `end`-aligned scroll-snap marker sits after Group Details' own content so scrolling all the way down (past the member row, to Leave Squad) has a legitimate third snap position instead of springing back to the top of Group Details.

`ChatSquadDetailBar` (`src/features/chat/components/header/ChatSquadDetailBar.tsx`) itself just shows the crew image + uppercase name + `Lv.{level} · {icon} {memberCount}` metadata row, a non-scrolling row of **online-only** member avatars (offline members omitted entirely; online dot `#66bb6a`) capped to ~6 visible via `maxWidth: 164` + `overflow-hidden`, and a static `ChevronUp` swipe-gesture hint. All Notif/Library/MagicEdit/Squads/Leave actions live in this sheet instead.
