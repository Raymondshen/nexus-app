...
1. **Group card details** (Figma 596:8296) — hero + invite section combined into one rounded (`var(--x3)`, all four corners) `--color-surface-sheet` card:
   - **Hero** (`aspect-ratio: 393/240`) — background + gradient overlay (`var(--gradient-image-overlay)`, unchanged); top row: action buttons only, right-aligned — `MagicEdit` (creator) + `Bell`/`BellOff` (opens `NotifSheet`, owned by `ChatInput`) + `Library` (navigates to `/chat/${crewId}/definitions`); bottom row: 40px `GroupAvatar` + crew name (uppercase, `--color-secondary`) + `{totalMessages} total Squad msg.` + XP progress bar. The old `Lv.{n} · {count} members` line and the numeric XP-fraction text are gone — Figma's heading row doesn't show them (crew level is still visible on the collapsed `ChatSquadDetailBar` below).
