...
- **`SocialLinksRow`** (`src/shared/components/ui/SocialLinksRow.tsx`) is the sole renderer of a profile's 5 social-link columns — text-only links (no icons; pixelarticons has no Instagram/X/Reddit/LinkedIn brand marks and nothing else in the app renders icon-only external links), `target="_blank" rel="noopener noreferrer"`, renders nothing if every link is null. Rendered in the same seam on both profile display pages — right after the hero's closing `</div>`, before `{status && <TickerBanner .../>}` — in `ProfileClient.tsx` and `AccountPageMember.tsx`.
