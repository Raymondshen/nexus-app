...
│   │   │                       (Group Details only — see its own section), SquadsListShared
│   │   │                       (Squads-row/Notifications/RoomPinSheet pieces shared with
│   │   │                       ChatSquadsPage)
│   │   ├── messages/           MessageList, MessageBubble, LinkPreviewCard
│   │   ├── sheets/             SquadDetailCard (shared by ChatRoomBrowseSheet's Group
│   │   │                       Details), PinDurationSheet, PinListSheet,
...
│   │                           ChatSquadsPage (room quick-switch + Notifications, `/chat/[crewId]/squads`)
...
### ChatRoomBrowseSheet — Group Details (`src/features/chat/components/input/ChatRoomBrowseSheet.tsx`, Figma 642:7848)
Solely the current room's own Group Details now — card + member row + Leave Squad. Quick-switching rooms and the Notifications feed used to live in this same overlay (Figma 589:3619) but moved to their own standalone page, `ChatSquadsPage` (see its own section below), reached via `ChatFloatingNav`'s Menu button — that split is why this component's props shrank to just `visible`/`squadDetail`/`currentUserId`/`allMuted`/`onEditSquad`/`onNotif`/`onLibrary`/`onClose`, with no `rooms`/`onSelectRoom`/`onCreateSquad`/`onPinCrew`/`onLeaveRoom` anymore. Opened two ways, both toggling the same sheet: a vertical swipe-up anywhere on `chatInputContainer` (`ChatInput`'s `handleTopPanEnd`), or a plain tap anywhere on `ChatSquadDetailBar` (`onTap`, wired to `setShowRoomBrowser((prev) => !prev)` in `ChatInput`) — a second tap while it's open closes it. Renders nothing at all when `squadDetail` is null (the DM screen — a DM has no group/squad to show details for; this used to still open showing Notifications+Squads content, which no longer lives here either).
...
**Header row** (`flex-shrink-0`, never scrolls): crew name (`PageHeader variant="sheet" titleColor="primary"` — bold DM Sans, `--color-primary`, no decorative leading icon) + the action icon row, right-aligned, always visible/tappable (the old scroll-position fade — `viewingGroupDetails`, dimming MagicEdit/Library until the user scrolled past a since-removed Notifications/Squads page — no longer applies, since Group Details is the only content left): `MagicEdit` (creator only) → `Bell`/`BellOff` (opens `NotifSheet`, owned by `ChatInput`) → `Library` (navigates to `/chat/${crewId}/definitions`) → `Close`. This exact order (`MagicEdit, Bell, Library, Close`) was verified against Figma 642:7848 by diffing each icon's exported SVG path against its pixelarticons equivalent — a genuine reorder from the previous `MagicEdit, Library, Bell, Close`.

**Body** is plain `overflow-y-auto` (no `scroll-snap-type` — that existed only to snap between the now-removed Notifications+Squads page and this one), just the card + member row + Leave Squad button in normal document flow, no "Group Details" section label above the card (Figma goes straight from header to card):
1. **Group card details** (`<SquadDetailCard>`) — hero + invite section combined into one rounded (`var(--x3)`, all four corners) `--color-surface-sheet` card:
   - **Hero** (`aspect-ratio: 393/240`) — background + gradient overlay (`var(--gradient-image-overlay)`); a single bottom-anchored row: 40px `GroupAvatar` + crew name (uppercase, `--color-secondary`) + `{totalMessages} total Squad msg.` + XP progress bar.
   - **Invite section** — `<InviteCodeCard>` (Figma 438:8098; same shared component as `MessageList`'s empty state — don't re-inline this markup a third time). Its border is purple (`border-purple`), matching Figma's `border-[#a855f7]` spec on both call sites.
2. **Member row** (`<SquadMemberRow>`, no "Members" label — Figma doesn't show one) — a horizontally-scrollable row (`overflow-x-auto no-scrollbar`) of member `UserCard`s (180px wide, every card the same total height by construction — see below). Each card: the member's own `profiles.background_url` as the header image (fallback `/img/default_image.png`, same as `AccountPageMember`/`MemberProfileClient` — NOT the crew's `background_image_url`), rendered as a plain `<img>` with `inset: 0`, `width/height: 100%`, `objectFit: cover` — same full-bleed fill technique as `ProfileHeroBackground` (user profile) and `ManageUserProfile`'s hero preview, all three now on `cover` since the underlying `supabaseImageLoader` non-proportional-resize bug was fixed at the loader level — see the `image-handling` skill + 32px `UserAvatar` (online dot if online) → username → class row (`Crown` 12×12 amber if creator + 12×12 `PixelSprite` (scale `0.5625`) + `{class} · {msgCount} msg.`) → a **reserved vinyl pill slot**: `<LinkPill type="vinyl">` (`src/shared/components/ui/LinkPill.tsx`, shared with `MessageBubble`'s header row) if the member has one, else a blank 20px-tall placeholder (`VINYL_PILL_HEIGHT`, matching `LinkPill`'s own natural height) → a **reserved status ticker slot**: `<TickerBanner>` (see below) if the member has a `status` set, else `BlankTickerSlot` — a placeholder that mirrors `TickerBanner`'s own outer wrapper markup so its rendered height matches exactly. Figma 432:7827 (the real mockup row) shows this precisely: the full card (432:7819, vinyl + ticker) and the `self-stretch` empty card (432:8008) both resolve to 231px, while the one card missing `self-stretch` (432:8021) collapses to 172px — the "collapse" look was the bug, not the design; every card must reserve both slots' space regardless of content, not rely on the row's `align-items: stretch` to paper over the difference. Tapping a card calls `onTapMember` → `/chat/${crewId}/member/${memberId}`.
3. **Leave Squad button** — kept as a plain flex child below the member row (no docked `SheetFooter` — this overlay has no sheet chrome to dock a footer against).

`ChatSquadDetailBar` (`src/features/chat/components/header/ChatSquadDetailBar.tsx`) itself just shows the crew image + uppercase name + `Lv.{level} · {icon} {memberCount}` metadata row (replacing the former "{N} Member online" line — online status is now conveyed purely by the avatar row's green dots, not restated in text), a non-scrolling row of **online-only** member avatars (offline members are omitted entirely, not just deprioritized; online dot `#66bb6a` always shown) capped to ~6 visible at once via `maxWidth: 164` + `overflow-hidden` (extra avatars are clipped, not scrollable), and a static (non-animated) `ChevronUp` swipe-gesture hint — no accompanying text label; the old "Swipe / view" text + tick-triggered pulse animation were both dropped. All Notif/Library/MagicEdit/Leave actions live in this sheet instead.

Invite is surfaced only via the inline `<InviteCodeCard>` in the group card details block.
...
### ChatSquadsPage (`src/features/chat/screens/ChatSquadsPage.tsx`, route `/chat/[crewId]/squads`, Figma 589:3617)
Standalone page reached by tapping `ChatFloatingNav`'s Menu icon — a real Next.js route rather than another client-side overlay, since `ChatFloatingNav` is a server-composed sibling of `ChatInput` in `chat/[crewId]/page.tsx`, not its parent, so it has no access to `ChatInput`'s own overlay state. This is the room quick-switch + Notifications content `ChatRoomBrowseSheet` used to render alongside Group Details before that sheet was simplified (see its own section above) — reuses `SwipePreviewCard` as-is plus `NotificationPreviewCard`/`NoNotificationsCard`/`ScrollEqualizerBars`/`RoomPinSheet`/`BrowseRoom`/`BrowseItem`/card-layout constants, all exported from `src/features/chat/components/input/SquadsListShared.tsx` (moved there once `ChatRoomBrowseSheet` stopped needing them itself) rather than duplicated.

Server page (`chat/[crewId]/squads/page.tsx`) fetches every non-DM crew this user belongs to with the same `RoomMeta` shape `SwipePreviewCard` needs (level, member count, background photo, unread count) — mirrors `home/page.tsx`'s batched-query structure, extended with `ensureRoomMeta.ts`'s own background-photo/presence fields Home's query doesn't need. Deliberately uncached (unlike Home's `unstable_cache`-wrapped queries) — this page is reached far less often and its whole point is surfacing what's current. Member rows are fetched lean (`crew_id, user_id`, no profile join) — profiles (`username`/`avatar_url`) are looked up in a second, separate query scoped to just the online subset, since only online members ever need that data for the online-avatar stack.

Header: `PageHeader variant="sheet" titleColor="primary"` — title "Squads", `right` = a plain Close (X). New element vs. the old combined sheet: an "Invite Friends" button (plain outlined-purple `Button`, not the richer `InviteCodeCard`/`ShareModal` treatment) — copies the same invite message those two use ("Come join my squad on Nexus app {code}"), for the current room's own invite code, with a brief "Copied!" label swap.

Selecting a DIFFERENT room preserves `ChatInput`'s swipe-nav peek/ghost transition (`ChatRoomPeekLayer`) — the only other trigger for it was `ChatRoomBrowseSheet`'s own room-card tap (`commitRoomSwitch`, removed from `ChatInput` along with the rest of its Squads-row plumbing once the split happened), so this page reimplements that same short sequence directly against the shared `chatRoomPeekStore`: `setPeek({phase:'committing'})` + `skipNextSlideEnter(true)` + `sessionStorage.setItem('nexus_chat_from', 'chat')` + `router.push`. `chatRoomPeekStore.roomMeta` is also seeded here for every room in the list on mount via `setRoomMetaBulk` (one batched store write, not a per-room loop — `roomMeta` is a single object every subscriber, e.g. `ChatRoomPeekLayer`, reads whole, so N separate `set()` calls would cost N re-renders instead of one) — this page's server-fetched data already has everything `RoomMeta` needs, so the peek ghost's room-name label resolves immediately even for a room the user has never opened this session.

Long-press (hold) a room card opens the Pin/Leave Squad sheet (`RoomPinSheet`, from `SquadsListShared.tsx`) — this page owns its own copy of the last-member-leaves-deletes-the-squad warning gate (`leaveTarget`/`showLastMemberWarning`), calling `pinCrewAction`/`leaveCrewAction` directly (client → server action, no `ChatInput` intermediary needed).
...
Surfaced two places: sorted first (right after the ever-first Create Squad card) in `ChatSquadsPage`'s Squads row — with its own gradient-heart badge (Figma 602:4170/606:3894, `SwipePreviewCard`'s `pinned` prop) — and preferred by `home/page.tsx`'s server-side launch redirect (see below) over that redirect's most-recent/only-crew fallback (now mostly a safety net given the invariant above), so relaunching the app lands straight in the pinned squad instead of Home. `pinned` only drives that badge now, not the card's border — a pinned-but-not-open card gets the same plain `--color-border-hover` border as any other card; only `selected` (the room you actually have open) drives the border, in `--color-primary`, matching `ScrollEqualizerBars`' (`SquadsListShared.tsx`) current-room bar color.

Opened by a 500ms long-press on a room card in `ChatSquadsPage` (same threshold as `ChatSheetReact`'s own long-press sheets) — `RoomPinSheet` (`SquadsListShared.tsx`, Figma 605:3830): a "What would you like to do?" header, a Pin Squad `SheetActionButton` (disabled + tertiary-colored icon/label when the card is already pinned) with an explanatory caption, and a Leave Squad `SheetActionButton` below it that works for ANY room card in the list — `ChatSquadsPage` calls `pinCrewAction`/`leaveCrewAction` directly for whichever card was long-pressed, generalized beyond just the currently-open room.
