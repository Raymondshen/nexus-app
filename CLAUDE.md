...
**Drag-to-reorder**: tap-and-hold specifically the `Menu` (≡) handle icon — same 500ms threshold as every other long-press in the app — starts a real `Reorder.Item` drag via `dragListener={false}` + manually-invoked `dragControls.start(e)` (the same technique `useSheetDrag` already uses for the sheet's own pull-to-close gesture). `e.stopPropagation()` on the handle's pointerdown keeps this from also firing the row's OWN separate long-press-for-actions gesture (which still opens `VinylActionSheet` — pin/remove/open-link — from anywhere else on the row). Auto-saves on every reorder step via `reorderNotesAction` (`notes/actions.ts`), which renumbers the *entire* list to `position = 0..N-1` in one pass rather than computing a fractional/between-neighbor value — deliberately simple since a personal vibes list is small. `useVibesState` (`VibesGrid.tsx`) is the shared hook backing both this and the (now-orphaned) `VibesGrid` grid — `orderedVinyls`/`pinnedId`/`addVibe`/`togglePin`/`remove`/`reorder`, so pin/remove/reorder logic isn't duplicated across the two renderings of the same data.
...
│                                VibesPlaylistSheet, VinylComboArt, AddVibeForm, VibesGrid (shared
│                                useVibesState/helpers only — its own grid component is orphaned),
│                                FloatingViewPill (orphaned)
├── shared/
