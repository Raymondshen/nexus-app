...
- **Every navigation, including the ones deliberately excluded from `nexus-pages-v3` above (`/home`, `/`, `/login`, `/onboarding/*`), is still intercepted** — those get a plain NetworkOnly fetch (never cached, so no stale-HTML-replay risk) that falls back to `/offline.html` on failure, instead of being left completely unhandled. `/offline.html` itself is precached into `nexus-pages-v3` on every `install` — the generated `sw.js`/`fallback-*.js` (workbox) precaches it via `next.config.ts`'s `pwaConfig.fallbacks`, but that worker is never registered (see below), so `sw-push.js` has to do this itself or the fallback resolves to `undefined` and `event.respondWith(undefined)` produces the browser's own bare native error page ("This page couldn't load") instead of anything this app controls — this is what a transient network failure right as a new Vercel deploy cuts over used to look like before this was added.
