...
interface PollCreatorSheetProps {
  ...
  userProfile: Pick<
    Profile,
    "id" | "username" | "avatar_class" | "avatar_url" | "status"
  >;
  onClose: () => void;
...
export function PollCreatorSheet({
  ...
  function updateOption(idx: number, value: string) {
    setOptions((prev) =>
      prev.map((o, i) => (i === idx ? value.slice(0, 100) : o)),
    );
  }
    ...
    try {
      ...
      const messageWithProfile: MessageWithProfile = {
        ...
        profile: {
          ...
          avatar_class: userProfile.avatar_class,
  ...
  const activeHint =
    DURATION_OPTIONS.find((d) => d.value === duration)?.hint ?? "";
            ...
            {options.length < 5 && (
              ...
              >
                <PlusBox
                  ...
                  aria-hidden="true"
                />
                <span className="font-silkscreen text-[11px] text-purple leading-none">
                  Add option
                </span>
              </button>
            )}
