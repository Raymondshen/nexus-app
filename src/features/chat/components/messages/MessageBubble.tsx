    ...
    >
      ...
      >
        <p
          ...
          style={{ fontSize: 7, color: bossColor }}
        >
          ⚠ BOSS RAID BEGINS
        </p>
        <p
        ...
        <p
          className="font-silkscreen leading-none"
          style={{ fontSize: 8, color: "var(--color-tertiary)" }}
        >
          {Math.round(liveHp).toLocaleString()} /{" "}
          {Math.round(maxHp).toLocaleString()} HP · 7-day window
        </p>
        {/* Live HP bar */}
        <div className="w-full" style={{ marginTop: 2 }}>
          <div
            className="relative w-full overflow-hidden"
            style={{ height: 8, background: "#2a1545", borderRadius: 2 }}
          >
            <div
              style={{
                position: "absolute",
...
function SystemMessage({ message }: { message: MessageWithProfile }) {
  ...
  if (content.startsWith("BOSS_SPAWN:"))
    return <BossSpawnMessage content={content} />;
