...
// Tapping anywhere in the backdrop area (outside the hero/invite card and the
// member row) calls `onClose`, matching the browse sheet's "tap outside to
// dismiss" behavior; both of those content blocks stop click propagation so
// taps inside them don't also bubble into the backdrop's onClose.
//
...
export function SquadDetailsSheet({
  ...
  return (
    <motion.div
      ref={sheetRef}
      className="fixed left-0 right-0 top-0 bg-black/85 flex flex-col overflow-y-auto nexus-scroll"
      style={{
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Action buttons */}
          <div className="relative flex items-center justify-end w-full" style={{ gap: 16 }}>
