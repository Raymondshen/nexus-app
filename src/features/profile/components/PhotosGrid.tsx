...
function PhotoCell({
  ...
  return (
    <div
      ...
      style={{ aspectRatio: '1', background: 'var(--color-surface)', minWidth: 0 }}
    ...
    >
      ...
      <AnimatePresence>
        {showActions && (
          <PhotoActionSheet
            isOwner={isOwner}
            onView={() => window.open(photo.url, '_blank', 'noopener,noreferrer')}
            onRemove={onRemove}
...
function AddPhotoCell({ onClick, uploading }: { onClick: () => void; uploading: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={uploading}
      ...
      {uploading ? (
        ...
        <div className="relative overflow-hidden flex-shrink-0" style={{ width: 24, height: 24 }}>
          <div className="absolute" style={{ inset: '16.67%' }}>
            <svg viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden style={{ width: '100%', height: '100%' }}>
  ...
  async function handleCropConfirm(area: Area, img: HTMLImageElement) {
    ...
    try {
      ...
      const { error: storageErr } = await supabase.storage
        .from('profile-photos')
        .upload(storageKey, blob, { contentType: blob.type, cacheControl: '31536000' })

      if (storageErr) throw new Error(storageErr.message)

      const { data: { publicUrl } } = supabase.storage.from('profile-photos').getPublicUrl(storageKey)

      const result = await addPhotoAction(publicUrl, storageKey)
      if (result.error || !result.photo) throw new Error(result.error ?? 'Failed to save photo')
        ...
        <div className="flex flex-col w-full" style={{ gap: 4 }}>
          {rows.map((row, ri) => (
            <div key={ri} className="flex w-full" style={{ gap: 4 }}>
              ...
              {row.length === 1 && <div className="flex-1 min-w-0" style={{ aspectRatio: '1' }} />}
              {row.length === 1 && <div className="flex-1 min-w-0" style={{ aspectRatio: '1' }} />}
              {row.length === 2 && <div className="flex-1 min-w-0" style={{ aspectRatio: '1' }} />}
            </div>
          ))}
        </div>
      </div>
