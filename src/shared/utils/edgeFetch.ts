...
// award-friendship-xp verify the caller's identity server-side (the anon key
// passes verify_jwt but carries no user, so it is rejected).
// Returns null when there's no session (caller should treat as a no-op).
//
// Deliberately NO retry: award-xp is not idempotent — a retry after a lost
// response could double-award XP. The AbortController only prevents a hung
// request on a bad connection from dangling forever.
